...
class Paciente extends Model
{
    ...
    public function scopeNombre($query, $nombre)
    {
        if(trim($nombre) != null)
        {
              $query->Join('personas', 'pacientes.idPersona', '=', 'personas.idPersona')
                  ->select(['pacientes.idPersona', 'personas.Persona_Nombre', 
                            'personas.Persona_Apellido', 'pacientes.condicion_llegada', 'pacientes.ubicacion'])
                  ->where('personas.Persona_Nombre','like','%'. $nombre .'%');
